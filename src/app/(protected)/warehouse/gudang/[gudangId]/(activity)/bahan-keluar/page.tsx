...
function BahanKeluar() {
  ...
  const {
    ...
    loadingNextPage,
    materialInfinite,
  } = useListMaterialExport({
    limit: 10,
  });

  const materialExportData = materialInfinite?.pages.flatMap(
    (page: any) => page?.data
  );
  ...
  async function handleDelete(id: number) {
    try {
      ...
      queryClient.invalidateQueries({
        queryKey: ["MATERIAL_EXPORTS", gudangId],
      });
    } catch (error) {
