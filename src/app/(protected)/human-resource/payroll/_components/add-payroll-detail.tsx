...
function AddPayrollDetail({ detailId }: { detailId: number }) {
  ...
  const onSubmit = async (val: any) => {
    try {
      ...
      await PayrollService.createDetail({
        ...val,
        payroll_id: detailId,
      });

      queryClient.invalidateQueries({
        queryKey: ["PAYROLL", detailId.toString()],
      });

      toast.success("Detail payroll berhasil dibuat");
      form.resetFields();
      modal.onClose();
    } catch (error) {
